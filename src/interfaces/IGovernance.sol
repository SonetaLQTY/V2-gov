...
interface IGovernance {
    ...
    event AllocatePairSonata(
        address indexed user,
        address indexed initiative,
        int256 deltaVotePairSonata,
        int256 deltaVetoPairSonata,
        uint256 atEpoch,
    ...
    struct Allocation {
        uint256 votePairSonata; // PairSonata allocated vouching for the initiative
        uint256 voteOffset; // Offset associated with PairSonata vouching for the initiative
        uint256 vetoPairSonata; // PairSonata vetoing the initiative
        uint256 vetoOffset; // Offset associated with PairSonata vetoing the initiative
        uint256 atEpoch; // Epoch at which the allocation was last updated
    }

    struct UserState {
        uint256 unallocatedPairSonata; // PairSonata deposited and unallocated
        uint256 unallocatedOffset; // The offset sum corresponding to the unallocated PairSonata
        uint256 allocatedPairSonata; // PairSonata allocated by the user to initatives
        uint256 allocatedOffset; // The offset sum corresponding to the allocated PairSonata
    }

    struct InitiativeState {
        uint256 votePairSonata; // PairSonata allocated vouching for the initiative
        uint256 voteOffset; // Offset associated with PairSonata vouching for to the initative
        uint256 vetoPairSonata; // PairSonata allocated vetoing the initiative
        uint256 vetoOffset; // Offset associated with PairSonata veoting the initative
        uint256 lastEpochClaim;
    }

    struct GlobalState {
        uint256 countedVotePairSonata; // Total PairSonata that is included in vote counting
        uint256 countedVoteOffset; // Offset associated with the counted vote PairSonata
    }

    /// @notice Returns the user's state
    /// @return unallocatedPairSonata PairSonata deposited and unallocated
    /// @return unallocatedOffset Offset associated with unallocated PairSonata
    /// @return allocatedPairSonata allocated by the user to initatives
    /// @return allocatedOffset Offset associated with allocated PairSonata
    function userStates(address _user)
        external
        view
        returns (
            uint256 unallocatedPairSonata,
            uint256 unallocatedOffset,
            uint256 allocatedPairSonata,
            uint256 allocatedOffset
        );
    /// @notice Returns the initiative's state
    /// @param _initiative Address of the initiative
    /// @return votePairSonata PairSonata allocated vouching for the initiative
    /// @return voteOffset Offset associated with votePairSonata
    /// @return vetoPairSonata PairSonata allocated vetoing the initiative
    /// @return vetoOffset Offset associated with vetoPairSonata
    /// @return lastEpochClaim // Last epoch at which rewards were claimed
    function initiativeStates(address _initiative)
        external
        view
        returns (
            uint256 votePairSonata,
            uint256 voteOffset,
            uint256 vetoPairSonata,
            uint256 vetoOffset,
            uint256 lastEpochClaim
        );
    /// @notice Returns the global state
    /// @return countedVotePairSonata Total PairSonata that is included in vote counting
    /// @return countedVoteOffset Offset associated with countedVotePairSonata
    function globalState() external view returns (uint256 countedVotePairSonata, uint256 countedVoteOffset);
    /// @notice Returns the amount of voting and vetoing PairSonata a user allocated to an initiative
    /// @param _user Address of the user
    /// @param _initiative Address of the initiative
    /// @return votePairSonata PairSonata allocated vouching for the initiative
    /// @return voteOffset The offset associated with votePairSonata
    /// @return vetoPairSonata allocated vetoing the initiative
    /// @return vetoOffset the offset associated with vetoPairSonata
    /// @return atEpoch Epoch at which the allocation was last updated
    function pairSonataAllocatedByUserToInitiative(address _user, address _initiative)
        external
        view
        returns (
            uint256 votePairSonata,
    ...
    /// @param _pairSonataAmount Amount of PairSonata to withdraw
    /// @param _recipient Address to which the tokens should be sent
    function withdrawPairSonata(uint256 _pairSonataAmount, address _recipient) external;
    ...
    /// @param _pairSonataAmount Amount of PairSonata associated with the entity
    ...
    ///   - 4% of the total voting PairSonata in the previous epoch
    ...
    /// @notice Allocates the user's PairSonata to initiatives
    /// @dev The user can only allocate to active initiatives (older than 1 epoch) and has to have enough unallocated
    /// PairSonata available, the initiatives listed must be unique, and towards the end of the epoch a user can only maintain or reduce their votes
    ...
    /// @param _absolutePairSonataVotes PairSonata to allocate to the initiatives as votes
    /// @param _absolutePairSonataVetos PairSonata to allocate to the initiatives as vetos
    ...
    /// @notice Deallocates the user's PairSonata from initiatives
    /// @param _initiativesToReset Addresses of initiatives to deallocate PairSonata from
    /// @param _checkAll When true, the call will revert if there is still some allocated PairSonata left after deallocating
